...
public class WorldCommand implements ICommand {
	...
	@Override
	public int compareTo(@NotNull Object arg) {
	...
	public void processCommand(ICommandSender sender, String[] string) {
				...
				if(stack != null && stack.getItem() instanceof ItemData) {
					...
					if(string.length >= 2) {
						try {
							...
							sender.addChatMessage(new ChatComponentText("Did you mean: /advRocketry" + string[0] + " [datatype] [amountFill]"));
							sender.addChatMessage(new ChatComponentText("Not a valid datatype"));
							@NotNull StringBuilder value = new StringBuilder();
							for(DataType data : DataType.values())
								if(!data.name().equals("UNDEFINED"))
									value.append(data.name().toLowerCase()).append(", ");
		...
		if(string.length >= 1 &&  string[0].equalsIgnoreCase("showEntityList")) {
			try {
				...
				EntityList.stringToClassMapping.keySet().forEach(name->sender.addChatMessage(new ChatComponentText((String) name)));
		...
		if(string.length > 1) {
			...
			if(string[0].equalsIgnoreCase("goto") && (string.length == 2 || string.length == 3)) {
						...
						try {
							...
							if(DimensionManager.getInstance().isDimensionCreated(deletedDimId)) {
								...
								if(net.minecraftforge.common.DimensionManager.getWorld(deletedDimId) == null || net.minecraftforge.common.DimensionManager.getWorld(deletedDimId).playerEntities.isEmpty()) {
									...
									for(Object player : net.minecraftforge.common.DimensionManager.getWorld(deletedDimId).playerEntities) {
										if(player instanceof EntityPlayer)sender.addChatMessage(new ChatComponentText(((EntityPlayer) player).getDisplayName()));
									}
				...
				//
				//			int planetId = starId;
				//			if(moon) starId = DimensionManager.getInstance().getDimensionProperties(planetId).getStarId();
				//
                ...
                if(string.length > 3) {
					if(string[1].equalsIgnoreCase("get")) {
						try {
							...
							@Nullable StellarBody star =  DimensionManager.getInstance().getStar(id);
							if(star == null)
